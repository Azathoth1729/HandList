 ...
 */
class PurePath(inputPathStr: String) {
    private val pathComponents: List<String> =
        inputPathStr.filterNot { it.isWhitespace() }.split("/").filter { it != "" }

    val isAbsolute: Boolean = inputPathStr.startsWith("/")
    val nameCount = pathComponents.size

    private val prefix = if (isAbsolute) "/" else ""
    ...
    val parent
        get() = PurePath(prefix + pathComponents.dropLast(1).joinToString("/"))
    ...
    fun resolve(other: PurePath): PurePath {
        ...
        if (other.pathStr == "") return this
        return PurePath(this.pathStr + "/" + other.pathStr)
    }
